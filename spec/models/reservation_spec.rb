...
RSpec.describe Reservation, type: :model do
  ...
  describe '#no_reservation_overlap' do
    it 'should add an error to the reservation when the same room is reserved twice' do
      ...
      existing_reservation = create(:reservation, room: room, check_in: Date.today, check_out: Date.tomorrow)
      reservation = build(:reservation, room: room, check_in: Date.today, check_out: Date.tomorrow + 1)
      reservation.save
    ...
    it 'should not add an error to the reservation' do
      room = create(:room) # Use FactoryBot create to create a room
      reservation = build(:reservation, room: room, check_in: Date.today, check_out: Date.tomorrow)
      reservation.save
